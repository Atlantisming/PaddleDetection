...
#     __shared__ = ['num_classes', 'use_focal_loss']
#     __inject__ = ['matcher', 'matcher_ori']
#
#     def __init__(self,
#                  num_classes=80,
#                  matcher='OVHungarianMatcher',
#                  matcher_ori='HungarianMatcher',
#                  loss_coeff={
#                      'class': 3,
#                      'bbox': 5,
#                      'giou': 2,
...
#             #     target_label.reshape([-1, 1]), index, updates.astype('int64'))
#             target_label = paddle.scatter(
#                 target_label.reshape([-1, 1]), index, paddle.zeros_like(updates))
#
#             target_label = target_label.reshape([bs, num_query_objects])
#         if self.use_focal_loss:
#             target_label = F.one_hot(target_label,
...
#         }
#
#     def _get_loss_bbox(self, outputs, inputs, match_indices, num_gts,
#                        postfix=""):
#         # boxes: [b, query, 4], gt_bbox: list[[n, 4]]
#         name_bbox = "loss_bbox" + postfix
#         name_giou = "loss_giou" + postfix
#         boxes = outputs['pred_boxes']
#         gt_bbox = inputs['boxes']
#         if boxes is None:
#             return {name_bbox: paddle.zeros([1]), name_giou: paddle.zeros([1])}
#         loss = dict()
#         if sum(len(a) for a in gt_bbox) == 0:
...
#         return loss
#
#     def _get_loss_embed(self, outputs, inputs,
#                         match_indices, num_gts, postfix=""):
#         name_embed = "loss_embed" + postfix
#         pred_embed, select_id, clip_query = outputs['pred_embed'], outputs['select_id'], outputs['clip_query']
#         gt_class = inputs['labels']
...
#
#         target_feature = []
#         for t, (_, i) in zip(inputs, match_indices):
#             for c in t["labels"][i]:
...
#         loss[name_embed] = loss[name_embed].sum() / num_gts
#         return loss
#
#
#     def _get_loss_mask(self, masks, gt_mask, match_indices, num_gts,
...
#
#         total_loss.updata(
#             self._get_loss_embed(pred_embed[
#                 -1] if pred_embed is not None else None, select_id, clip_query,
